...
# description - no blank field
        if not post_data['desc']:#python returns an empty string as false
...
# status and errors are keys in the dictionary response_to_views
        if errors: #failed validations
            ...
            user = self.create(name = post_data['name'], email = post_data['email'], password = hashed_password, desc = post_data['desc'])
